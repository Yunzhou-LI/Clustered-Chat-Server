...
bool Redis::connect()
{
    ...
    if(_publish_context==nullptr)
    {
        cerr<<"publish_context connect error"<<endl;
        return false;
    }
    ...
    if(_subscribe_context==nullptr)
    {
        cerr<<"subscribe_context connect error"<<endl;
        return false;
    }
    //连接过后，在单独的线程中监听通道上的事件，有消息就给业务层进行上报
    thread t([&](){
...
{
    //SUBSCRIBE命令本身会造成线程阻塞等待通道里面发生的消息，这里只做订阅，不负责接收通道发来的消息
...
// 在独立线程中接收来自已订阅通道中的消息   因为订阅后会一直阻塞等待，所以这里新开线程来等待消息
void Redis::observe_channel_message()
{
    redisReply *reply = nullptr;
    while(REDIS_OK == redisGetReply(this->_subscribe_context,(void**)&reply))
    {
        //订阅收到的消息通常有三个，一起封装在element数组
